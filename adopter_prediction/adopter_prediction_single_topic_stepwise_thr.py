...
#train and test model on each topic in training, add predicted users into query set stepwise and train on distances of candidates from bigger query set
...
print vec_file, num_init_adopters, metric_Hausdorff_m_avg, top_k, train_ex_limit, top_k_test, max_step, confidence_thr
...
with open("adopter_pred_files/sequence_file_split_indices_weight_n"+str(num_init_adopters)+".pickle","wb") as fd:
	pickle.dump(train_seq_id_weight,fd)
	pickle.dump(test_seq_id_weight,fd)
"""
with open("/mnt/filer01/word2vec/degree_distribution/adopter_pred_files/sequence_file_split_indices_weight_n"+str(num_init_adopters)+".pickle","rb") as fr:
...
for i in train_seq_id_weight:
	...
	X, Y, cand_user_idx, cc, cr = get_cand_feature_vectors(init_adopters, seq_sample_vocab, N)
	cand_cov+=cc
	cand_cr+=cr
	...
	for train_ind,test_ind in kf:
		...
		while num_predicted < top_k_test:
			...
			#precision at k
			if count==max_step:
				pred_adopters_ind = [w for w,_ in nlargest(top_k_test-num_predicted,cand_prob_list,key=lambda x: x[1])]
			else:
				pred_adopters_ind = [w for w,p in cand_prob_list if p >= thr]
			pred_adopters = cand_user_idx[pred_adopters_ind]
			pred_adopters_ind_total+=pred_adopters_ind
			num_predicted += len(pred_adopters_ind)


			if num_predicted<top_k_test and len(pred_adopters_ind)!=0:
				...
				thr-=0.1
				# count=9
			print count, num_predicted, top_k_test-num_predicted, len(train_X_new[0]), len(train_X_new), len(pred_adopters_ind), len(pred_adopters_ind_total), len(set(pred_adopters_ind_total)), len(test_ind_new), len(test_X_new[0]), len(test_X_new), len(set(pred_adopters_ind+test_ind_new))

		pred_adopters_ind_total = pred_adopters_ind_total[:top_k_test]
	...
	print "Avg precision", precision_k, "Avg recall", recall_k, "precision total", sum(prec_k_total)*1./(l+1), "recall total", sum(rec_k_total)*1./(l+1), "cc", cc, "cr", cr, l
	l+=1
	if l%20==0:
		print "example num", l
	if l==train_ex_limit:
...
# with open("adopter_pred_files/single_topic_train_test_files/eval_n"+str(num_init_adopters)+"_rf_prec"+str(top_k_test)+"_step"+str(max_step)+".pickle","wb") as fd:
...
print vec_file, num_init_adopters, metric_Hausdorff_m_avg, top_k, top_k_test, max_step, confidence_thr