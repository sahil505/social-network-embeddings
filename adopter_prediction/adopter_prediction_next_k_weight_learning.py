...
#train on increasing number of initial adopters and test model on next k adopters for each topic in training
...
for i in train_seq_id_weight:
	...
	for init_size in range(num_init_adopters,next_k_limit,next_k_test):	
		init_adopters=seq_sample_vocab[init_size-num_init_adopters:init_size]
		next_k_adopters = set(seq_sample_vocab[init_size:init_size+next_k_test])
		M = len(next_k_adopters)
		N = top_k #1000 #M #num_users

		X, Y, cand_user, cc, cr = get_cand_feature_vectors(init_adopters, next_k_adopters, N)
		cand_cov.append(cc)
		cand_cr.append(cr)
		...
		if init_size!=num_init_adopters:

			print "check score", clf_t.score(prevX,prevY), clf_t.score(X,Y)
			...
			#precision at k
			pred_adopters = [w for w,_ in nlargest(next_k_test,cand_prob_list,key=lambda x: x[1])]
			num_hits = len(set(next_k_adopters)&set(pred_adopters))
			prec_k_cv = num_hits*1./next_k_test
			...
			print "precision", prec_k_cv, "recall", rec_k_cv, "num adopters", M, len(X), "cc", cc, "cr", cr
		...
		clf_t = init_clf()
		clf_t.fit(X, Y)
		print "fit for", init_size, "score", clf_t.score(X,Y)
		prevX=X
		prevY=Y

	prec_k_total.append(precision_k)
	rec_k_total.append(recall_k)
	cand_set_recall.append(cand_cov)
	cand_set_cr.append(cand_cr)
	cand_set_size_list.append(len(X))
	...
	if l==train_ex_limit:
		break
print "training examples taken", l, "avg candidate set recall", numpy.mean(cand_set_recall, axis=0), "avg candidate set class ratio", numpy.mean(cand_set_cr, axis=0)
...
with open("adopter_pred_files/single_topic_next_k/eval_n10_rf_k10.pickle","wb") as fd:
