#get followers of ninitial adopters of a hashtag sequence in test sequences, rank them using user vectors and compare with actual adopters in the sequence
...
par_m = 8
metric_Hausdorff_m_avg = 0 # 0-Hausdorff, 1-m, 2-avg
top_k = 500
seq_len_threshold = 500 #top_k
norm_vec = True
next_k = 2
stepwise_pred = False
print vec_file, num_init_adopters, metric_Hausdorff_m_avg, top_k, norm_vec, stepwise_pred, next_k
...
def get_Nranked_list_fol_user_vector(query_set,next_adopters,N):
	candidates = set()
	for a in query_set:
		followers = getadj(a)
		candidates.update(followers)
	candidates=candidates-set(query_set)
	cand_set_size = len(candidates)
	num_adopters = len(candidates&next_adopters)
	M = len(next_adopters)
	cc = 0.0
	if M>0:
		cc = num_adopters*1./M
	...
	try:
		...
		print "query word not present"
		return
	for pres_word in candidates:
		cand_index = vocab_index[pres_word]
		pres_vec = vec[cand_index]
		dist_k = [0.0]*set_size
		k=0
		for voc_ind in query_set_ind:
			user_vec = vec[voc_ind]
		...
		# 		break
	wordN = [w for w,_ in nsmallest(N,dist_total,key=lambda x: x[1])]
	return wordN,cc,cand_set_size,num_adopters #zip(wordN,distN)
...
for i in seq_random_index:
	...
	adopters_vec,cc,cand_set_size,op = get_Nranked_list_fol_user_vector(init_adopters,seq_sample_vocab,N)
	...
	rec_k = num_hits/M
	mean_cc+=cc
	mean_cand_size+=cand_set_size
	print "Avg precision", average_precision, "Precision", prec_k, "Recall", rec_k, "users not found", not_found, "adopters in seq", len(seq_sample_vocab), "cc", cc, "avg", mean_cc*1./num_seqs, "op", op, "from", M, "cand size", cand_set_size
	...
	mean_rec_k+=rec_k
	cc_total.append(cc)
	cand_set_size_total.append(cand_set_size)
	...
	if seq_count_limit==0:
		break
print "number of seq considered", num_seqs, "avg cc", mean_cc*1./num_seqs
...
print "cc", [numpy.mean(cc_total), numpy.std(cc_total), numpy.median(cc_total)]
print "avg cand set", [numpy.mean(cand_set_size_total), numpy.std(cand_set_size_total), numpy.median(cand_set_size_total)]
