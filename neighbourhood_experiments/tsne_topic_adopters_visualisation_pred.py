...
def save_embed_plot(Y,color,tag,prec,fname):
	fig = Plot.figure()
	init = []
	tp = []
	fn = []
...
if __name__ == "__main__":
	...
	for i in range(0,train_ex_limit):
		...
		# with open("/mnt/filer01/word2vec/degree_distribution/adopter_pred_files/single_topic_vis/"+fname+".pickle","wb") as fd:
			# pickle.dump(Y,fd)
		save_embed_plot(Y,color,tag,prec,'embed_adopters_topic_rf'+str(i))
		save_embed_plot(Y,color_log,tag,prec_log,'embed_adopters_topic_lr'+str(i))
