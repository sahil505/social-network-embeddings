...
NUM_PROCESSES = 5
adj_file = "/mnt/filer01/word2vec/degree_distribution/sample_adj_list.csv"
...
def read_adoption_sequence(adoption_sequence_filename, start, end,train_seq_id):
...
def write_adj_list(adj_list,adj_file):
	with open(adj_file,'wb') as fd:
		for i in range(0,len(adj_list)):
			fd.write(str(i))
			if len(adj_list[i])>0:
				fd.write(";"+";".join(map(str,adj_list[i]))+"\n")
			else:
				fd.write("\n")

#get sentences from hashtag sequences read from input file from start to end line numbers
def get_sentences(adoption_sequence_filename,start_line_num,end_line_num,process_num,train_seq_id):
	tag_count = 0
	for t,segment in read_adoption_sequence(adoption_sequence_filename,start_line_num,end_line_num,train_seq_id):
		...
		if len(segment)>50 and len(segment)<100:
			print t,len(segment)
			adj_list, rev_adj_list = get_hashtag_graph_adj(segment)
			write_adj_list(adj_list,adj_file)
			sys.exit(0)
		else:
			continue
...
def write_sentence(process_num,start,end,train_seq_id):
	try:	
		with open(out_dir+'/hashtagAdoptionSentences'+str(process_num)+'.txt','wb') as fd:
			start_time = datetime.datetime.now()
			sentences = get_sentences(adoption_sequence_filename,start,end,process_num,train_seq_id)
			for s in sentences:
				fd.write(" ".join(s)+"\n")
				# count[len(s)]+=1
...
write_sentence(0,0,10000,range(0,10000))
"""
...
for s,e in ( (i,min(i+lines_per_process,NUM_LINES)) for i in xrange(0,NUM_LINES,lines_per_process) ):
	pool.apply_async(write_sentence, args=(process_num,s,e,train_seq_id))
	process_num+=1
pool.close()
pool.join()
"""